...
/**
 * Generate a file containing hosts with specified format.
 *
 * @param {String} format: Output format
 * @param {Function} cb: standard cb(err, result) callback
 */
Breaker.prototype.gen = function (format, cb) {
  var formatter = require('./formatters/' + format),
    conf = JSON.parse(fs.readFileSync(p.join(process.cwd(), '.breaker.json')));

  console.log('Generating %s file: %s', format, formatter.file);
  fs.writeFile(p.join(process.cwd(), formatter.file), formatter.format(conf), cb);
};
